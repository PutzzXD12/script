-- ================== DRIP CLIENT V7.5 (FIXED - AUTO LOGIN) ==================
-- Version: 7.5 (Khusus HP - Simple Fly + Chams Hologram)
-- Developer: Putzz XD
-- FIX: Auto login dengan key PutzzVIP
    ...
    if success and data then
        ...
        if success2 and jsonData then
            for _, keyData in pairs(jsonData) do
                local dbKey = keyData.key or ""
                if string.upper(dbKey) == string.upper(inputKey) then
                    return true, "KEY VALID! (" .. (keyData.jenis or "PERMANEN") .. ")"
                end
            end
        end
    end
    ...
    UserInputService.TouchMoved:Connect(function(input, gameProcessed)
        ...
        if touching and touchStartPos then
            ...
            if math.abs(delta.Y) > 15 then
                verticalControl = delta.Y < 0 and 1 or -1
            else
                verticalControl = 0
            end
            if math.abs(delta.X) > 15 then
                horizontalControl = delta.X < 0 and -1 or 1
            else
                horizontalControl = 0
            end
            touchStartPos = input.Position
...
local function applyChamsToPlayer(player)
    ...
    local connection = RunService.RenderStepped:Connect(function()
        ...
        if not player or not player.Parent then connection:Disconnect() return end
...
local function removeChamsFromPlayer(player)
    ...
    if originalDataChams[player] then
        for part, data in pairs(originalDataChams[player]) do
            if part and part.Parent then pcall(function() part.Material = data.Material part.Transparency = data.Transparency part.Color = data.Color end) end
        end
...
local function applyChamsToAllPlayers()
    for _, player in pairs(Players:GetPlayers()) do if player ~= LocalPlayer then applyChamsToPlayer(player) end end
end

local function removeChamsFromAllPlayers()
    for player, _ in pairs(chamsConnections) do removeChamsFromPlayer(player) end
end

local function toggleChams(state)
    chamsEnabled = state
    if state then applyChamsToAllPlayers() else removeChamsFromAllPlayers() end
end

-- ================== FUNGSI CROSSHAIR ==================
local function createCrosshair()
    if crosshairObject then pcall(function() crosshairObject:Destroy() end) crosshairObject = nil end
...
local function createPlayerCounter()
    if enemyCountText then pcall(function() enemyCountText:Remove() end) enemyCountText = nil end
...
Players.PlayerRemoving:Connect(function(player)
    if ESPTable[player] then
        for _, drawing in pairs(ESPTable[player]) do pcall(function() if drawing and drawing.Remove then drawing:Remove() end end) end
        ESPTable[player] = nil
    end
    if SkeletonESP[player] then
        for _, lineData in pairs(SkeletonESP[player]) do pcall(function() if lineData[1] and lineData[1].Remove then lineData[1]:Remove() end end) end
        ...
        pcall(function()
            for player, drawings in pairs(ESPTable) do
                if not player or not player.Parent then
                    for _, drawing in pairs(drawings) do if drawing and drawing.Remove then drawing:Remove() end end
                    ESPTable[player] = nil
                end
            end
            for player, lines in pairs(SkeletonESP) do
                if not player or not player.Parent then
                    for _, lineData in pairs(lines) do if lineData[1] and lineData[1].Remove then lineData[1]:Remove() end end
                    SkeletonESP[player] = nil
                end
            end
        end)
...
local function setupChamsEvents()
    Players.PlayerAdded:Connect(function(player)
        if chamsEnabled and player ~= LocalPlayer then
            player.CharacterAdded:Connect(function() task.wait(0.5); if chamsEnabled then applyChamsToPlayer(player) end end)
            if player.Character then task.wait(0.5); applyChamsToPlayer(player) end
        end
    end)
    Players.PlayerRemoving:Connect(function(player) removeChamsFromPlayer(player) end)
end
setupChamsEvents()
...
local function loadMainScript()
    ...
    local function createTab(name, icon, idx)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            for _, child in pairs(content:GetChildren()) do
                if child:IsA("Frame") then height = height + child.Size.Y.Offset + 10 end
            end
    ...
    -- TAB MAIN
    createToggle(tabMain, "FLY", false, function(s) if s then flyEnabled = true startFlyMode() else flyEnabled = false stopFlyMode() end end)
    ...
    createToggle(tabMain, "Crosshair", false, function(s) if s then createCrosshair() else removeCrosshair() end end)
    ...
    createToggle(tabESP, "Hologram", false, function(s) toggleChams(s) end)
    
    -- TAB UTILITY
    createToggle(tabUtility, "God Mode", false, function(s)
        antiDamageEnabled = s
        if s then setupAntiDamage() else
            if antiDamageHeartbeat then antiDamageHeartbeat:Disconnect() end
            if antiDamageConnection then antiDamageConnection:Disconnect() end
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        ...
        if chamsEnabled then task.wait(0.5); applyChamsToAllPlayers() end
    end)
    ...
    createButton(tabColor, "Ungu (Default)", function() changeTheme(Color3.fromRGB(156, 39, 176)) end)
    createButton(tabColor, "Cyan", function() changeTheme(Color3.fromRGB(0, 255, 255)) end)
    createButton(tabColor, "Merah", function() changeTheme(Color3.fromRGB(255, 0, 0)) end)
    ...
    copyTiktokBtn.MouseButton1Click:Connect(function()
        pcall(function()
            if setclipboard then setclipboard("Putzz_mvpp") end
        end)
    end)
    copyWaBtn.MouseButton1Click:Connect(function()
        pcall(function()
            if setclipboard then setclipboard("088976255131") end
        end)
    end)
    ...
    openBtn.MouseButton1Click:Connect(function()
        ...
        if menuOpen then
            ...
            TweenService:Create(mainFrame, TweenInfo.new(0.25), {Position = UDim2.new(0.5, -200, 0.5, -275)}):Play()
        else
            TweenService:Create(mainFrame, TweenInfo.new(0.25), {Position = UDim2.new(0.5, -200, 1, 0)}):Play()
            task.wait(0.25)
...
-- ================== AUTO LOGIN DENGAN KEY PutzzVIP ==================
task.wait(1)
if KeyTextBox then
    KeyTextBox.Text = "PutzzVIP"
    task.wait(0.5)
...
print("Auto login dengan key: PutzzVIP")